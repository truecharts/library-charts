...
*/}}
{{- define "tc.common.v10.spawner.rbac" -}}
  ...
  {{- range $name, $rbac := .Values.rbac }}
    {{- if $rbac.enabled -}}
      ...
      {{- if and (not $rbacValues.nameOverride) (ne $name (include "tc.common.v10.lib.util.rbac.primary" $)) -}}
        {{- $_ := set $rbacValues "nameOverride" $name -}}
      {{ end -}}

      {{- $_ := set $ "ObjectValues" (dict "rbac" $rbacValues) -}}
      {{- include "tc.common.v10.class.rbac" $ }}
    {{- end }}
  {{- end }}
{{- end }}
