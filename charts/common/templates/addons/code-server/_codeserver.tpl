...
*/}}
{{- define "tc.common.v10.addon.codeserver" -}}
{{- if .Values.addons.codeserver.enabled -}}
  {{/* Append the code-server container to the additionalContainers */}}
  {{- $container := include "tc.common.v10.addon.codeserver.container" . | fromYaml -}}
  {{- if $container -}}
    {{- $_ := set .Values.additionalContainers "addon-codeserver" $container -}}
  {{- end -}}

  {{/* Include the deployKeySecret if not empty */}}
  {{- $secret := include "tc.common.v10.addon.codeserver.deployKeySecret" . -}}
  {{- if $secret -}}
    {{- $secret | nindent 0 -}}
  {{- end -}}

  {{/* Append the secret volume to the volumes */}}
  {{- $volume := include "tc.common.v10.addon.codeserver.deployKeyVolumeSpec" . | fromYaml -}}
  {{- if $volume -}}
  ...
  {{- if .Values.addons.codeserver.ingress.enabled -}}
    ...
    {{- $svcName := printf "%v-codeserver" (include "tc.common.v10.names.fullname" .) -}}
